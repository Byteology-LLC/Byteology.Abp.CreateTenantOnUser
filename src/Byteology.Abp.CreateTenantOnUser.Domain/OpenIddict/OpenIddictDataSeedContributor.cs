...
namespace Byteology.Abp.CreateTenantOnUser.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("CreateTenantOnUser") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "CreateTenantOnUser", DisplayName = "CreateTenantOnUser API", Resources = { "CreateTenantOnUser" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["CreateTenantOnUser_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["CreateTenantOnUser_Web:RootUrl"]!.EnsureEndsWith('/');

            /* CreateTenantOnUser_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["CreateTenantOnUser_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["CreateTenantOnUser_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["CreateTenantOnUser_Swagger:RootUrl"]?.TrimEnd('/');
